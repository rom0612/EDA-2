#include <stdio.h>
#include "ordenamientos.h"
#include "utilerias.h"
#include "proyecto1.h"


void selectionSort(int arreglo[], int n) {
    int indiceMenor, i, j;
    for(i = 0; i < n - 1; i++) {
        indiceMenor = i;
        for(j = i + 1; j < n; j++) {
            if(arreglo[j] < arreglo[indiceMenor])
                indiceMenor = j;
        }
        if(i != indiceMenor) {
            swap(&arreglo[i], &arreglo[indiceMenor]);
        }

        printf("\n -- Iteracion numero %d: ", i + 1);
        PrintArray(arreglo, n);
    }
}

void InsertionSort(int lista[], int n) {
    for (int i = 1; i < n; i++) {
        int index = lista[i];
        int j = i - 1;
        while (j >= 0 && lista[j] > index) {
            lista[j + 1] = lista[j];
            j = j - 1;
            printf("\n -- Iteracion numero %d: ", i + 1);
            PrintArray(lista, n);
        }
        lista[j + 1] = index;
    }
}

void BubbleSort(int lista[], int n) {
    for (int i = 0; i < n - 1; i++) {
        int swapped = 0; //NUEVA VARIABLE
        for (int j = 0; j < n - i - 1; j++) {
            if (lista[j] > lista[j + 1]) {
                swap(&lista[j], &lista[j + 1]);
                swapped = 1; //SI DE HIZO UN CAMBIO, EL VALOR DE SWAPPED CAMBIA
            }
        }
        
        printf("\n -- Iteracion numero %d: ", i + 1);
        PrintArray(lista, n);
        if (swapped == 0) { //SI NO SEHIZO NINGUN CAMBIO D POSICIÓN YA ESTÁ ORDENADO
            printf("\nla lista ya está ordenada, después de la iteración %d: ", i + 1);
            break;
        }
    }
}

void quickSort(int arr[], int low, int high){
    if (low < high) {
        printf("\nLlama a QuickSort: ");
        printSubArray(arr, low, high);
        
        int pi = partition(arr, low, high);
        
        printf("Dividiendo: \n");
        printf("-> Sublista izquierda: ");
        printSubArray(arr, low, pi - 1);
        printf("-> Sublista derecha: ");
        printSubArray(arr, pi + 1, high);
        printf("\n");
        
        quickSort(arr, low, pi - 1);
        quickSort(arr, pi + 1, high);
    }
}

void HeapSort(int* A, int size){
    int heapSize = size - 1; // local en lugar de global
    
    BuildHeap(A, size, &heapSize); //pasar heapSize
    
    printf("=== ELIMINAR LA RAÍZ ===\n");
    for(int i = size - 1; i > 0; i--){
        printf("intercambio de raiz (%d) con ultimo elemento (%d)\n", A[0], A[i]);
        swap(&A[0], &A[i]);
        heapSize--;
        printf("Heap después del intercambio: ");
        PrintArray(A, size);
        Heapify(A, 0, size, heapSize); // pasar heapSize
        printf("Heap después de reordenar: ");
        PrintArray(A, size);
        printf("\n");
    }
}
// ... implementaciones de algoritmos ...

void probarAlgoritmos() {
    int tamanos[] = {SIZE_50, SIZE_100, SIZE_500, SIZE_800, SIZE_1000, SIZE_2000, SIZE_5000, SIZE_10000};
    int numTamanos = sizeof(tamanos) / sizeof(tamanos[0]);
    
    // Nombres de algoritmos
    char* nombres[] = {"Selection Sort", "Insertion Sort", "Bubble Sort", "Quick Sort", "Heap Sort", "Merge Sort"};
    // Wrapper functions for quickSort and mergeSort
    void quickSortWrapper(int arr[], int n, int* op) {
        quickSort(arr, 0, n - 1);
    }
    void mergeSortWrapper(int arr[], int n, int* op) {
        mergeSort(arr, 0, n - 1);
    }

    // Punteros a funciones
    void (*funciones[])(int[], int, int*) = {selectionSort, InsertionSort, BubbleSort, 
        quickSortWrapper,
        HeapSort,
        mergeSortWrapper
    };
    int numAlgoritmos = sizeof(*funciones) / sizeof(funciones[0]);

    printf("Tamanio\t");
    for (int i = 0; i < numAlgoritmos; i++) {
        printf("%s\t", nombres[i]);
    }
    printf("\n");
    
    for (int t = 0; t < numTamanos; t++) {
        int size = tamanos[t];
        int* arr = (int*)malloc(size * sizeof(int));
        
        // Generar arreglo aleatorio
        for (int i = 0; i < size; i++) {
            arr[i] = rand() % MAX;
        }
        
        printf("%d\t", size);
        
        // Probar cada algoritmo
        for (int a = 0; a < numAlgoritmos; a++) {
            int* copia = (int*)malloc(size * sizeof(int));
            copiarArreglo(arr, copia, size);
            
            int operaciones = 0;
            funciones[a](copia, size, &operaciones);
            
            printf("%d\t", operaciones);
            free(copia);
        }
        
        printf("\n");
        free(arr);
    }
}